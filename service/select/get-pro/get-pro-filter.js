...
var filter = {
    filter: function (data1,data2,data3,data4,data5, callback) {
        var sql = `SELECT
        property.pro_id,
        property.pro_head,
        type_pro.type_id,
        ...
        INNER JOIN zone ON province.zone_id = zone.zone_id
				WHERE   price BETWEEN ${ data1 } and ${ data2 } OR 
					    pro_area BETWEEN ${ data3 } and ${ data4 } OR
						pro_bedroom = ${ data5 }`
    

        return db.query(sql,  [data1,data2,data3,data4,data5] , callback);
    }
